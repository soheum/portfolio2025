...
const Box12: React.FC<Box12Props> = ({ progress }) => {
  ...
  function updateValueFromGraph(index: number) {
    ...
    const num = Math.round(minNum + normalized * (maxNum - minNum));
    animateValue(num);
  }
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (dragging) {
        ...
        const topPadding = 2;
        const bottomPadding = 2;
        const minYCoord = topPadding;
        const maxYCoord = viewBoxHeight - bottomPadding;
        const yClamped = Math.min(maxYCoord, Math.max(minYCoord, viewBoxY));
        ...
        // Handle hover only inside the graph bounds
        const graphRect = graphRef.current.getBoundingClientRect();
        const { clientX, clientY } = e;

        const withinX = clientX >= graphRect.left && clientX <= graphRect.right;
        const withinY = clientY >= graphRect.top && clientY <= graphRect.bottom;
        if (!withinX || !withinY) return;

        const graphRelativeX = clientX - graphRect.left;
        let t = graphRelativeX / graphRect.width;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <g ref={verticalLinesBackgroundRef} id="vertical-lines-background"></g>
        <g ref={verticalLinesRef} id="vertical-lines"></g>
      ...
      <div ref={dividerRef} id="divider"></div>
    </div>
  );
};
